...
const fallbackContent = {
  hero: {
    ...
    imageSrc: { url: "/hero-slider4.jpg", alt: "About Background" }
  },
  story: {
    subtitle: "Who We Are",
    ...
    buttonLink: "/products",
    imageSrc: { url: "/napierStems.webp", alt: "Super Napier Grass" },
...
export default function AboutPage() {
  ...
  useEffect(() => {
    pagesApi.getPageContent("about")
      .then(res => {
        if (res.data && res.data.success && res.data.page) {
          const page = res.data.page;
          setContent(page.content || fallbackContent);
          // Apply fallback values if individual SEO fields are empty
          setSeo({
      ...
      .catch(err => {
        console.warn("AboutPage CMS endpoint offline, using default fallback content.", err);
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  if (loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#FAFCF8]">
        <div className="flex flex-col items-center">
          <Loader2 className="h-12 w-12 animate-spin text-[#16a34a] mb-4" />
    ...
    >
      ...
      <section className="relative overflow-hidden rounded-b-[2rem] bg-gray-900 text-white pt-32 pb-28 px-4">
        ...
        <div className="absolute inset-0 z-0">
          <img
            src={content.hero?.imageSrc?.url || "/hero-slider4.jpg"}
            alt={content.hero?.imageSrc?.alt || "About Background"}
      ...
      <section className="py-24 px-4 relative overflow-hidden bg-[#FAFCF8]">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            >
              <p className="text-xs font-bold text-[#059669] uppercase tracking-widest mb-2">
                {content.story?.subtitle || "Who We Are"}
              </p>
              <h2 className="text-3xl md:text-5xl font-black text-gray-900 uppercase tracking-tight leading-tight mb-6 break-all">
                {content.story?.title?.split(" ").slice(0, -1).join(" ") || "Rooted In"}{' '}
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#4ade80] to-[#16a34a]">
                  {content.story?.title?.split(" ").slice(-1).join(" ") || "Purpose"}
                </span>
              </h2>
              ...
              {content.story?.buttonLink && (
                <Link to={content.story.buttonLink}>
                  <button className="bg-[#111] hover:bg-black text-white px-8 py-4 rounded-full font-bold transition-all shadow-lg hover:scale-105 flex items-center gap-2">
                    {content.story.buttonText || "Explore Products"} <ArrowRight className="w-4 h-4" />
                  </button>
                </Link>
              )}
            </motion.div>
            ...
            >
              <div className="rounded-3xl overflow-hidden shadow-2xl">
                <img
                  src={content.story?.imageSrc?.url || "/napierStems.webp"}
                  alt={content.story?.imageSrc?.alt || "Super Napier Grass"}
              ...
              <div className="absolute -bottom-6 -left-6 bg-white rounded-2xl shadow-xl px-6 py-4 flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs text-gray-400 font-semibold uppercase tracking-wider">
                    {content.story?.badgeTitle || "Germination Rate"}
                  </p>
                  <p className="text-2xl font-black text-gray-900">
                    {content.story?.badgeValue || "95%+"}
                  </p>
                </div>
              </div>
            </motion.div>
      ...
      <section className="py-24 px-4 bg-[#FAFCF8] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <motion.div
            ...
            className="text-center mb-14"
          >
            <p className="text-xs font-bold text-[#059669] uppercase tracking-widest mb-2 font-sans">What Drives Us</p>
            <h2 className="text-3xl md:text-5xl font-black text-gray-900 uppercase tracking-tight">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {content.values?.map(({ iconName, title, desc }, i) => {
              const Icon = iconMap[iconName] || Sprout;
              const valueStyles = [
                { bg: 'bg-[#f2fae6]', accent: 'text-[#16a34a]', iconBg: 'bg-[#dcfce7]' },
                { bg: 'bg-[#fff4ed]', accent: 'text-orange-500', iconBg: 'bg-orange-100' },
                { bg: 'bg-[#eef8ed]', accent: 'text-[#059669]', iconBg: 'bg-[#d1fae5]' }
              ];
              ...
              return (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  whileHover={{ y: -6 }}
                  className={`${style.bg} rounded-3xl p-8 border border-white/60 shadow-sm hover:shadow-xl transition-all`}
                >
        ...
        <section className="py-14 bg-[#FAFCF8] relative overflow-hidden">
          ...
          <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="bg-[#0f9c40] rounded-bl-[4rem] rounded-tr-[4rem] rounded-tl-2xl rounded-br-2xl py-20 px-8 relative overflow-hidden shadow-2xl">
              ...
              >
                <p className="text-xs font-bold text-[#dcfce7] uppercase tracking-widest mb-4">
                  {content.cta.subtitle}
                </p>
                <h2 className="text-3xl md:text-5xl font-black uppercase tracking-tight leading-tight mb-6 text-white">
                  {content.cta.title?.split(" ").slice(0, -2).join(" ") || "Join Our"}{' '}
                  <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-[#bbf7d0]">
                    {content.cta.title?.split(" ").slice(-2).join(" ") || "Growing Family"}
                  </span>
                </h2>
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                  {content.cta.button1Link && (
                    <Link to={content.cta.button1Link}>
                      <button className="bg-white text-[#16a34a] hover:bg-green-50 px-8 py-4 rounded-full font-bold transition-all shadow-lg hover:scale-105 flex items-center gap-2 w-full sm:w-auto justify-center">
                        {content.cta.button1Text} <ArrowRight className="w-4 h-4" />
                      </button>
                    </Link>
                  )}
                  {content.cta.button2Link && (
                    <a href={content.cta.button2Link}>
                      <button className="bg-white/10 hover:bg-white/20 border border-white/20 text-white px-8 py-4 rounded-full font-bold transition-all flex items-center gap-2 w-full sm:w-auto justify-center">
                        <PhoneCall className="w-4 h-4" /> {content.cta.button2Text}
                      </button>
                    </a>
                  )}
                </div>
